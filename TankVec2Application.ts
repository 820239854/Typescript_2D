...
class Tank {
    ...
    // 我们在Tank类中增加一个成员变量，用来标示Tank初始化时是否朝着y轴正方向
    ...
    public gunLength: number = Math.max(this.width, this.height); // 炮管长度，default情况下，等于坦克的width和height中最大的一个数值
    public gunMuzzleRadius: number = 5;

    public linearSpeed: number = 100.0;
    // turretRotateSpeed用于控制炮塔的旋转速度，初始化时为2角度对应的弧度
    ...
    public draw(app: TestApplication): void {
        ...
        // 炮口，先将局部坐标系从当前的方向，向x正方向平移炮管长度个单位，此时局部坐标系在炮管最右侧
        app.context2D.translate(this.gunLength, 0);
        // 然后再从当前的坐标系向x轴平移炮口半径单位，这样炮口的外切圆正好和炮管香接触
    ...
    public draw2(app: TestApplication): void {
        ...
        app.context2D.beginPath();
        if (this.initYAxis) {
            // 交换width和height，这样我们就不需要修改TestApplication中的示例代码了
        ...
        app.context2D.beginPath();
        if (this.initYAxis) {
            // 当我们朝着y轴正方向时，椭圆的radiuX < radiuY
            app.context2D.ellipse(0, 0, 10, 15, 0, 0, Math.PI * 2);
        } else {
            // 当我们朝着x轴正方向时，椭圆的radiuX > radiuY
            app.context2D.ellipse(0, 0, 15, 10, 0, 0, Math.PI * 2);
        }
        app.context2D.fill();
        ...
        app.context2D.moveTo(0, 0);
        if (this.initYAxis) {
            // 当我们朝着y轴正方向时，炮管是沿着y轴正方向绘制的
            app.context2D.lineTo(0, this.gunLength);
        } else {
            // 当我们朝着x轴正方向时，炮管是沿着x轴正方向绘制的
            app.context2D.lineTo(this.gunLength, 0);
        }
        app.context2D.stroke();
        ...
        app.context2D.save();
        if (this.initYAxis) {
            // 当我们朝着y轴正方向时，标记坦克前方的圆球是沿着y轴正方向绘制的
            app.context2D.translate(0, this.height * 0.5);
        } else {
            // 当我们朝着x轴正方向时，标记坦克前方的圆球是沿着x轴正方向绘制的
            app.context2D.translate(this.width * 0.5, 0);
        }
        app.fillCircle(0, 0, 10, 'green');
        app.context2D.restore();
...
class TestApplication extends Canvas2DApplication {
    ...
    public render(): void {
        if (this.context2D !== null) {
            ...
            this.drawTank();
            this.drawCoordInfo(
                '坐标 : [' + (this._mouseX - this.tank.pos.x).toFixed(2) + ',' + (this._mouseY - this.tank.pos.y).toFixed(2) + "] 角度 : " + Math2D.toDegree(this.tank.tankRotation).toFixed(2),
                this._mouseX,
                this._mouseY);
        }
    }
